...
export default function ApiKeysTab({ searchQuery, showToast }: ApiKeysTabProps) {
  ...
  return (
    <>
      ...
      <div className={`dash-card ${tableVisible ? "" : "dash-hidden"}`} data-search-terms="api keys table label key created status actions">
            ...
            <tbody>
              {keys.map((k) => (
                <tr key={k.id}>
                  ...
                  <td>
                    <span className={`dash-badge ${k.status === "Active" ? "dash-badge-success" : "dash-badge-warning"}`}>
                      {k.status}
                    </span>
                  </td>
                  <td>
      ...
      <div className={`dash-card ${helperVisible ? "" : "dash-hidden"}`} data-search-terms="api key storage rotation best practices">
        <h3 className="dash-card-title" style={{ marginBottom: "1rem" }}>Best Practices</h3>
        <div className="dash-settings-helper" style={{ marginBottom: "0.75rem" }}>
          <strong>How to store keys:</strong> Set your API key as an environment variable:
          <code style={{ display: "block", marginTop: "0.5rem", fontFamily: "'SF Mono', 'Fira Code', monospace", fontSize: "0.82rem", background: "var(--d-bg)", padding: "0.5rem 0.75rem", borderRadius: "6px", border: "1px solid var(--d-border)" }}>
            export AEGIS_API_KEY=ak_live_xxx
          </code>
        </div>
        <div className="dash-settings-helper">
          <strong>Key rotation:</strong> Rotate keys every 90 days. Use the Rotate action to generate a new key without downtime.
        </div>
      </div>

      {showCreateModal && (
        <div className="dash-modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) closeCreateModal(); }}>
          <div className="dash-modal">
            <div className="dash-modal-header">
              <h3 className="dash-modal-title">Create API Key</h3>
              <button className="dash-modal-close" onClick={closeCreateModal}>
            ...
            <div className="dash-modal-body">
              {newKeyRevealed === null ? (
                <>
                  ...
                  <div className="dash-form-group">
                    <label className="dash-label">Environment</label>
                    <select
                      className="dash-input"
                      value={newEnv}
                  ...
                  <div className="dash-form-group">
                    <label className="dash-label">Scopes</label>
                  ...
                  <div className="dash-form-group">
                    <label className="dash-label">Rate Limit</label>
                    <select
                      className="dash-input"
                      value={newRateLimit}
            ...
            <div className="dash-modal-footer">
              {newKeyRevealed === null ? (
                <>
                  <button className="dash-btn dash-btn-ghost" onClick={closeCreateModal}>Cancel</button>
                  <button className="dash-btn dash-btn-primary" onClick={handleCreate}>Create Key</button>
                </>
              ) : (
                <button className="dash-btn dash-btn-ghost" onClick={closeCreateModal}>Close</button>
              )}
            </div>
          </div>
        </div>
      ...
      {showRotateModal && (
        <div className="dash-modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) { setShowRotateModal(null); setNewKeyRevealed(null); } }}>
          <div className="dash-modal">
            <div className="dash-modal-header">
              <h3 className="dash-modal-title">Rotate API Key</h3>
              <button className="dash-modal-close" onClick={() => { setShowRotateModal(null); setNewKeyRevealed(null); }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
              </button>
            </div>
            <div className="dash-modal-body">
            ...
            <div className="dash-modal-footer">
              {newKeyRevealed === null ? (
                <>
                  <button className="dash-btn dash-btn-ghost" onClick={() => { setShowRotateModal(null); setNewKeyRevealed(null); }}>Cancel</button>
                  <button className="dash-btn dash-btn-primary" onClick={() => handleRotate(showRotateModal)}>Rotate</button>
                </>
              ) : (
                <button className="dash-btn dash-btn-ghost" onClick={() => { setShowRotateModal(null); setNewKeyRevealed(null); }}>Close</button>
              )}
            </div>
          </div>
        </div>
      ...
      {showDeleteModal && (
        <div className="dash-modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) setShowDeleteModal(null); }}>
          <div className="dash-modal">
            <div className="dash-modal-header">
              <h3 className="dash-modal-title">Delete API Key</h3>
              <button className="dash-modal-close" onClick={() => setShowDeleteModal(null)}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
              </button>
            </div>
            <div className="dash-modal-body">
            ...
            <div className="dash-modal-footer">
              <button className="dash-btn dash-btn-ghost" onClick={() => setShowDeleteModal(null)}>Cancel</button>
              <button className="dash-btn dash-btn-danger" onClick={() => handleDelete(showDeleteModal)}>Delete</button>
            </div>
          </div>
        </div>
      )}
